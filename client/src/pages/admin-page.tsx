...
import { UserRole, TutorApprovalStatus, UserStatus } from "@shared/schema";
...
export default function AdminPage() {
  ...
  const rejectTutorMutation = useMutation({
    mutationFn: async ({ userId, reason }: { userId: number; reason: string }) => {
      const res = await apiRequest("PATCH", `/api/admin/tutor-profiles/${userId}/reject`, { rejectionReason: reason });
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/tutor-profiles"] });
  ...
  const banUserMutation = useMutation({
    mutationFn: async ({ userId, reason }: { userId: number; reason: string }) => {
      const res = await apiRequest("PATCH", `/api/admin/users/${userId}/ban`, { banReason: reason });
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/users"] });
  ...
  const activateUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The user has been activated and can now use the platform.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  // Handle activating a banned user
                                ...
                                <TableCell>
                                  ...
                                  {user.status === UserStatus.BANNED && user.banReason && (
                                    <div className="text-xs text-gray-500 mt-1">
                                      Reason: {user.banReason.substring(0, 25)}{user.banReason.length > 25 ? '...' : ''}
                                    </div>
                                  )}
                                </TableCell>
                                <TableCell>
