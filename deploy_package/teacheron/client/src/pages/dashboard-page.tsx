...
export default function DashboardPage({ initialTab = "overview" }: DashboardPageProps) {
  ...
  const tutorProfileSchema = z.object({
    ...
    hourlyRate: z.string().optional().transform(val => val ? parseInt(val) : null),
  ...
  React.useEffect(() => {
    if (profileData && user?.role === UserRole.TUTOR) {
      ...
      form.reset({
        ...
        hourlyRate: profileData.hourlyRate ? profileData.hourlyRate.toString() : "",
        bio: profileData.bio || "",
  ...
  const onSubmitProfile = (data: z.infer<typeof tutorProfileSchema>) => {
    // Convert hourlyRate from string to number if present
    const formattedData = {
      ...data,
      hourlyRate: data.hourlyRate ? parseInt(data.hourlyRate.toString()) : null
    };
  ...
  const { data: jobsData, isLoading: isLoadingJobs } = useQuery({
    queryKey: [user?.role === UserRole.STUDENT ? "/api/jobs" : "/api/bids"],
    enabled: !!user,
  });
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="p-6 text-center border-b border-gray-200">
                  <Avatar className="h-20 w-20 mx-auto">
                    <AvatarImage 
                      src={profileData && profileData.profileImageUrl ? profileData.profileImageUrl : `https://api.dicebear.com/7.x/initials/svg?seed=${user.fullName}`} 
                    />
                      ...
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-2xl font-semibold">{jobsData?.length || 0}</p>
                          </div>
                        </div>
                      </div>
                      <div className="bg-white p-4 rounded-lg border border-gray-200">
                          ...
                          <Link key={conversation.userId} href={`/messages/${conversation.userId}`}>
                            <div className="flex items-center p-3 rounded-lg hover:bg-gray-50 cursor-pointer border border-gray-200">
                              <Avatar className="h-10 w-10">
                                <AvatarImage src={conversation.user?.profileImageUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${conversation.user?.fullName}`} />
                                <AvatarFallback>{conversation.user?.fullName?.substring(0, 2)}</AvatarFallback>
                              </Avatar>
                              <div className="ml-3 flex-1">
                                <div className="flex items-center justify-between">
                                  <p className="text-sm font-medium text-gray-900">{conversation.user?.fullName}</p>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                            <div className="relative avatar-upload-container">
                              <Avatar className="h-24 w-24">
                                <AvatarImage 
                                  src={form.watch('profileImageUrl') || `https://api.dicebear.com/7.x/initials/svg?seed=${user.fullName}`} 
                                />
                                <AvatarFallback>{user.fullName.substring(0, 2)}</AvatarFallback>
                              </Avatar>
                              <FormField
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-900">{profileData.hourlyRate ? `$${profileData.hourlyRate}/hour` : "Not specified"}</p>
                                </div>
                              </div>
                            </div>
                        ...
                        {profileData.reviews.map((review) => (
                          <div key={review.id} className="bg-white p-4 rounded-lg border border-gray-200">
                            <div className="flex items-start">
                              <Avatar className="h-10 w-10">
                                <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${review.student?.fullName}`} />
                                <AvatarFallback>{review.student?.fullName?.substring(0, 2)}</AvatarFallback>
                              </Avatar>
                              <div className="ml-3">
                                <div className="flex items-center">
                                  <p className="text-sm font-medium text-gray-900">{review.student?.fullName}</p>
